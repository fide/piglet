var Alloy = require('alloy'),
	...
	keyobject = require('framework/kernel/kernelKeyObject').keyobject;
...
function Sync(method, model, opts) {
	...
	if (method === 'read') {
		if (opts.parse) {
			// is collection
			var list = [];
			_.each(keyobject.getKeys(), function(prop) {
				var match = prop.match(regex);
				if (match !== null) {
					list.push(keyobject.getObject({key:prop}));
				}
			});
			resp = list;
		} else {
			// is model
			var obj = keyobject.getObject({key: prefix + '-' + model.id});
			model.set(obj);
			resp = model.toJSON();
		}
	}
		...
		keyobject.setObject({key:prefix + '-' + model.id, object: (model.toJSON() || {})});
		resp = model.toJSON();
	} else if (method === 'delete') {
		keyobject.removeProperty({key: prefix + '-' + model.id});
		model.clear();
		resp = model.toJSON();
	}
